...
$(document).ready(function () {
  ...
  if (openMenus) {
    openMenus.split(',').forEach(function (id) {
      $('#' + id).addClass('show');
  ...
  $('.collapse').on('shown.bs.collapse', function () {
    let current = localStorage.getItem('openMenus') || '';
    const open = current.split(',').filter(Boolean);
    if (!open.includes(this.id)) {
      open.push(this.id);
    }
    localStorage.setItem('openMenus', open.join(','));
  });

  $('.collapse').on('hidden.bs.collapse', function () {
    let current = localStorage.getItem('openMenus') || '';
    let open = current.split(',').filter(Boolean);
  ...
  $('.navbar-nav .nav-link').each(function () {
    ...
    if (linkPath && currentPath.startsWith(linkPath)) {
      ...
      if (collapse.length) {
        ...
        const toggleLink = $('[data-bs-target="#' + collapse.attr('id') + '"]');
